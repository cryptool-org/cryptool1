...
;General
  ...
  !define VersionInfo "1.4.41" ; no beta here to avoid line wrap in installer
  !define VersionFile "1_4_41" ; _beta_0x
  !define Publisher "CrypTool Team"
  
  Name "${ProgramName} ${VersionInfo}"
  OutFile "SetupCrypTool_${VersionFile}_${LANGUAGE_STR}.exe"
  BrandingText "(C) 1998-2017 CrypTool Team"
...
;Pages
  ...
  !if ${LANGUAGE_STR} == 'de' 
	!define MULTIUSER_INSTALLMODEPAGE_TEXT_TOP "Wδhlen Sie aus, ob Sie ${ProgramName} ${VersionInfo} nur fόr sich selbst oder fόr alle Benutzer dieses Systems installieren wollen. Klicken Sie auf Weiter, um fortzufahren."
  !endif
...
!if ${LANGUAGE_STR} == 'de' 
   ...
   !define SCN_SCRIPT		"CT-Buch"
   !define SCL_SCRIPT		"CT-Book-${LANGUAGE_STR}.pdf"
   !define SCN_PRESENTATION	"Prδsentation"
   !define SCL_PRESENTATION	"CrypTool1-Presentation-${LANGUAGE_STR}.pdf"
   ...
   !define UNINSTALL_PROMPT	"ist bereits installiert.$\n$\nBitte 'OK' klicken, um die vorherige Installation zu ersetzen; oder 'Abbrechen', um ein anderes Installationsverzeichnis zu wδhlen."
   !define UNINSTALL_PROMPT2    "Das Verzeichnis existiert bereits.$\n$\nBitte wδhlen Sie ein anderes, oder deinstallieren Sie das Programm in diesem Verzeichnis."
   !define UNINSTALL_CONFIRM	"Bei der Deinstallation werden alle Dateien im Programmverzeichnis gelφscht - auch von Ihnen erzeugte asymmetrische Schlόssel und gespeicherte Beispieldateien.$\n$\nWenn Sie die Deinstallation hier abbrechen, kφnnen Sie erzeugte Schlόssel mit dem bisher installierten CrypTool exportieren.$\n$\nSoll CrypTool wirklich deinstalliert werden?"
   !define UNINSTALL_RMPROGDIR_FAILED "Das Programmverzeichnis konnte nicht (vollstδndig) gelφscht werden.$\n$\nBitte lφschen Sie es manuell."
...
!else if ${LANGUAGE_STR} == 'pl'
   ...
   !define SCL_AES_TOOL		"aestool.exe"
   !define SCN_SCRIPT		"CT Book"
   !define SCL_SCRIPT		"CT-Book-${LANGUAGE_SUBST_STR}.pdf"
   !define SCN_PRESENTATION	"Presentation"
   !define SCL_PRESENTATION	"CrypTool1-Presentation-${LANGUAGE_SUBST_STR}.pdf"
   !define SCN_README		"ReadMe"
   !define SCL_README 		"ReadMe-${LANGUAGE_SUBST_STR}.txt"
   !define SCN_UNINSTALL	"Usunac"
   !define SCL_UNINSTALL	"Uninstall.exe"
   !define URL		"http://www.cryptool.pl/"
   !define UNINSTALL_PROMPT	"jest juΏ zainstalowany. $\n$\nKliknij 'OK', Ώeby usunΉζ poprzedniΉ wersje \
 lub 'Anuluj', aby okreliζ inny katalog instalacji."
   !define UNINSTALL_PROMPT2    "Taki katalog juΏ istnieje. $\n$\nWybierz inny katalog, albo odinstaluj program z tego katalogu."
   !define UNINSTALL_CONFIRM	"Odinstalowanie programu oznacza skasowanie wszystkich plikσw w katalogu instalacyjnym (takΏe wygenerowanych przez Ciebie kluczy asymetrycznych i zapisanych przyk³adσw).$\n$\n PrzerywajΉc ten proces moΏesz skopiowaζ te pliki w bezpieczne miejsce.$\n$\nCzy chcesz kontynuowaζ odinstalowywanie programu CrypTool?"
...
!else if ${LANGUAGE_STR} == 'es'
   ...
   !define SCN_NUMBERSHARK	"NumberShark"
   !define SCL_NUMBERSHARK	"NumberShark.exe"
   !define SCN_NUMBERSHARK_HELP "Ayuda de NumberShark"
   !define SCL_NUMBERSHARK_HELP "NumberShark-${LANGUAGE_SUBST_STR}.chm"
   !define SCN_AES_TOOL		"AES-Tool"
   !define SCL_AES_TOOL		"aestool.exe"
   !define SCN_SCRIPT		"CT Book"
   !define SCL_SCRIPT		"CT-Book-${LANGUAGE_SUBST_STR}.pdf"
   !define SCN_PRESENTATION	"Presentaciσn"
   !define SCL_PRESENTATION	"CrypTool1-Presentation-${LANGUAGE_STR}.pdf"
  ...
  versiσn anterior o 'Cancelar' para seleccionar otro directorio de instalaciσn."
   ...
   !define UNINSTALL_CONFIRM	"La desinstalaciσn eliminarα todos los archivos en este directorio - incluso clave asimιtricas que haya generado y archivos de ejemplo guardados.$\n$\nSi cancela ahora serα capaz de exportar las claves generadas en su instalaciσn actual de CrypTool.$\n$\nΏEstα seguro que desea continuar con el proceso de desinstalaciσn?"
...
!else if ${LANGUAGE_STR} == 'rs'
  ...
  prethodnu verziju ili 'Otkai' za odabir instalacionog direktorijuma."
...
!else if ${LANGUAGE_STR} == 'el'
   ...
   !define SCN_NUMBERSHARK	"NumberShark"
   !define SCL_NUMBERSHARK	"NumberShark.exe"
   !define SCN_NUMBERSHARK_HELP "Βοήθεια του NumberShark"
   !define SCL_NUMBERSHARK_HELP "NumberShark-${LANGUAGE_SUBST_STR}.chm"
   !define SCN_AES_TOOL		"AES-Tool"
   !define SCL_AES_TOOL		"aestool.exe"
