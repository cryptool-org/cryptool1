...
extern DWORDTF		cdecl TwofishAsmCodeSize(void);
#endif
...
#define	GetSboxKey	DWORDTF SKEY[4];	/* local copy */ \
...
#define	GetSboxKey	DWORDTF SKEY0	= key->sboxKeys[0]		/* local copy */
/*----------------------------------------------------------------*/
...
#ifdef GetCodeSize
extern DWORDTF Here(DWORDTF x);			/* return caller's address! */
DWORDTF TwofishCodeStart(void) { return Here(0); }
#endif
...
*
* Notes:  Note that the parameter d is a DWORDTF array, not a byte array.
...
*
-****************************************************************************/
int ParseHexDword(int bits,CONST char *srcTxt,DWORDTF *d,char *dstTxt)
	{
	int i;
	char c;
	DWORDTF b;
...
*
-****************************************************************************/
DWORDTF f32(DWORDTF x,CONST DWORDTF *k32,int keyLen)
	{
	BYTE  b[4];
	...
	*((DWORDTF *)b) = Bswap(x);	/* make b[0] = LSB, b[3] = MSB */
...
*
-****************************************************************************/
DWORDTF RS_MDS_Encode(DWORDTF k0,DWORDTF k1)
	{
	int i,j;
	DWORDTF r;
...
*
-****************************************************************************/
void ReverseRoundSubkeys(keyInstanceTwofish *key,BYTE newDir)
	{
	DWORDTF t0,t1;
	register DWORDTF *r0=key->subKeys+ROUND_SUBKEYS;
	register DWORDTF *r1=r0 + 2*key->numRounds - 2;
...
#define Xor32(dst,src,i) { ((DWORDTF *)dst)[i] = ((DWORDTF *)src)[i] ^ tmpX; } 
#define	Xor256(dst,src,b)				\
	{									\
	register DWORDTF tmpX=0x01010101u * b;\
	for (i=0;i<64;i+=4)					\
...
void Xor256(void *dst,void *src,BYTE b)
	{
	register DWORDTF	x=b*0x01010101u;	/* replicate byte to all four bytes */
	register DWORDTF *d=(DWORDTF *)dst;
	register DWORDTF *s=(DWORDTF *)src;
...
*
-****************************************************************************/
int reKey(keyInstanceTwofish *key)
	{
	int		i,j,k64Cnt,keyLen;
	int		subkeyCnt;
	DWORDTF	A=0,B=0,q;
	DWORDTF	sKey[MAX_KEY_BITS/64],k32e[MAX_KEY_BITS/64],k32o[MAX_KEY_BITS/64];
	...
	switch (keyLen)	/* case out key length for speed in generating S-boxes */
		...
		case 128:
			...
			#define	sb128(N) {					\
				...
				{ register DWORDTF k0=b##N(sKey[0]);	\
				for (i=0;i<256;i+=2) { one128(N,0); one128(N,1); } } }
#endif
		#elif defined(MIN_KEY)
		...
		case 192:
			...
			#define	sb192(N) {						\
				...
				{ register DWORDTF k0=b##N(sKey[0]);	\
				  register DWORDTF k1=b##N(sKey[1]);	\
...
*
-****************************************************************************/
int makeKeyTwofish(keyInstanceTwofish *key, BYTE direction, int keyLen,CONST char *keyMaterial)
	{
...
*
-****************************************************************************/
int cipherInitTwofish(cipherInstanceTwofish *cipher, BYTE mode,CONST char *IV)
	{
	int i;
#if VALIDATE_PARMS				/* first, sanity check on parameters */
	if (cipher == NULL)			
	...
	if ((mode != MODE_ECB) && (IV))	/* parse the IV */
		...
		for (i=0;i<BLOCK_SIZE/32;i++)	/* make byte-oriented copy for CFB1 */
			((DWORDTF *)cipher->IV)[i] = Bswap(cipher->iv32[i]);
		}
...
*
-****************************************************************************/
int blockEncryptTwofish(cipherInstanceTwofish *cipher, keyInstanceTwofish *key,CONST BYTE *input,
				int inputLen, BYTE *outBuffer)
	{
	int   i,n;						/* loop counters */
	DWORDTF x[BLOCK_SIZE/32];			/* block being encrypted */
	DWORDTF t0,t1;					/* temp variables */
	...
	DWORDTF sk[TOTAL_SUBKEYS];
	DWORDTF IV[BLOCK_SIZE/32];
	...
	if (mode == MODE_CFB1)
		...
		for (n=0;n<inputLen;n++)
			{
			blockEncryptTwofish(cipher,key,cipher->IV,BLOCK_SIZE,(BYTE *)x);
	...
	memcpy(sk,key->subKeys,sizeof(DWORDTF)*(ROUND_SUBKEYS+2*rounds));
	if (mode == MODE_CBC)
...
#endif
#define	LoadBlockE(N)  x[N]=Bswap(((DWORDTF *)input)[N]) ^ sk[INPUT_WHITEN+N] ^ IV[N]
...
#if LittleEndian
#define	StoreBlockE(N)	((DWORDTF *)outBuffer)[N]=x[N^2] ^ sk[OUTPUT_WHITEN+N]
#else
#define	StoreBlockE(N)	{ t0=x[N^2] ^ sk[OUTPUT_WHITEN+N]; ((DWORDTF *)outBuffer)[N]=Bswap(t0); }
#endif
		StoreBlockE(0);	StoreBlockE(1);	StoreBlockE(2);	StoreBlockE(3);
		if (mode == MODE_CBC)
			{
			IV[0]=Bswap(((DWORDTF *)outBuffer)[0]);
			IV[1]=Bswap(((DWORDTF *)outBuffer)[1]);
			IV[2]=Bswap(((DWORDTF *)outBuffer)[2]);
			IV[3]=Bswap(((DWORDTF *)outBuffer)[3]);
			}
#ifdef DEBUG
...
*
-****************************************************************************/
int blockDecryptTwofish(cipherInstanceTwofish *cipher, keyInstanceTwofish *key,CONST BYTE *input,
				int inputLen, BYTE *outBuffer)
	{
	int   i,n;						/* loop counters */
	DWORDTF x[BLOCK_SIZE/32];			/* block being encrypted */
	...
	for (n=0;n<inputLen;n+=BLOCK_SIZE,input+=BLOCK_SIZE/8,outBuffer+=BLOCK_SIZE/8)
		{
		DebugDump(input,"\n",rounds+1,0,0,0,1);
#define LoadBlockD(N) x[N^2]=Bswap(((DWORDTF *)input)[N]) ^ sk[OUTPUT_WHITEN+N]
...
#define		Decrypt2(R,id)	{ DecryptRound(2,R+1,id); DecryptRound(0,R,id); }
		...
		if (cipher->mode == MODE_ECB)
			{
#if LittleEndian
#define	StoreBlockD(N)	((DWORDTF *)outBuffer)[N] = x[N] ^ sk[INPUT_WHITEN+N]
#else
#define	StoreBlockD(N)	{ t0=x[N]^sk[INPUT_WHITEN+N]; ((DWORDTF *)outBuffer)[N] = Bswap(t0); }
#endif
...
#define	StoreBlockD(N)	x[N]   ^= sk[INPUT_WHITEN+N] ^ IV[N];	\
						IV[N]   = Bswap(((DWORDTF *)input)[N]);	\
						((DWORDTF *)outBuffer)[N] = Bswap(x[N]);
			StoreBlockD(0);	StoreBlockD(1);	StoreBlockD(2);	StoreBlockD(3);
#undef  StoreBlockD
			DebugDump(outBuffer,"",-1,0,0,0,1);
			}
