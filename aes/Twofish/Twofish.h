...
#define		BAD_DWORD_SIZE		-11 /* incorrect size of DWORDTF */
...
/* Typedefs:
	...
	Note:  See "PLATFORM.H" for definitions of BYTE and DWORDTF
*/
typedef DWORDTF fullSbox[4][256];
...
typedef struct 
	...
	DWORDTF keySig;					/* set to VALID_SIG by makeKey() */
	int	  numRounds;				/* number of rounds in cipher */
	DWORDTF key32[MAX_KEY_BITS/32];	/* actual key bits, in dwords */
	DWORDTF sboxKeys[MAX_KEY_BITS/64];/* key bits used for S-boxes */
	DWORDTF subKeys[TOTAL_SUBKEYS];	/* round subkeys, input/output whitening bits */
#if REENTRANT
...
#define	VALID_SIG	 0x504D4F43		/* 'COMP':  C is compiled with -DCOMPILE_KEY */
	DWORDTF cSig1;					/* set after first "compile" (zero at "init") */
	void *encryptFuncPtr;			/* ptr to asm encrypt function */
	void *decryptFuncPtr;			/* ptr to asm decrypt function */
	DWORDTF codeSize;					/* size of compiledCode */
	DWORDTF cSig2;					/* set after first "compile" */
...
typedef struct 
	...
	/* Twofish-specific parameters: */
	DWORDTF cipherSig;				/* set to VALID_SIG by cipherInit() */
	DWORDTF iv32[BLOCK_SIZE/32];		/* CBC IV bytes arranged as dwords */
	} cipherInstanceTwofish;

/* Function protoypes */
int makeKeyTwofish(keyInstanceTwofish *key, BYTE direction, int keyLen, char *keyMaterial);

int cipherInitTwofish(cipherInstanceTwofish *cipher, BYTE mode, char *IV);

int blockEncryptTwofish(cipherInstanceTwofish *cipher, keyInstanceTwofish *key, BYTE *input,
				int inputLen, BYTE *outBuffer);

int blockDecryptTwofish(cipherInstanceTwofish *cipher, keyInstanceTwofish *key, BYTE *input,
				int inputLen, BYTE *outBuffer);

int	reKey(keyInstanceTwofish *key);	/* do key schedule using modified key.keyDwords */
...
#define		Copy1(d,s,N)	((DWORDTF *)(d))[N] = ((DWORDTF *)(s))[N]
