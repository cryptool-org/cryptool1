...
// Globale Variablen
extern char *Pfad, *PseVerzeichnis; // siehe multipad.cpp
...
{
	return (CString)"CN="+Firstname + ((CString)" ") + Name + ((CString)" [") + CreatTime + ((CString)"], L=Frankfurt, O=DB, C=DE");
}
...
int CKeyFile::ExtractData(CString filename, char **ctstr, char **creatime, char **name, char **firstname, char **keyType, char **keyID)
{
	...
	// CDlgAsymKeyCreat::CCreateAsymKeys() erzeugt wurde,
