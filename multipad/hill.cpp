...
void hill::anlegen_mat (void)
{
	enc_mat = new square_matrix(dim, modul);
	dec_mat = new square_matrix(dim, modul);
}

// Speicher fuer Matrizen freigeben
void hill::freigeben_mat (void)
{
	if (enc_mat)
	{
		delete enc_mat;
...
void hill::init_zahlen_zeichen (char* erlaubte_zeichen)
{
...
hill::hill(char* erlaubte_zeichen)
{
...
hill::hill(char* erlaubte_zeichen, int d)
{
	init_zahlen_zeichen(erlaubte_zeichen);
	laPositionVektor = new long[modul];
	iaLinearKombination1 = new int[modul];
...
int hill::my_char_to_int (char c) const
{
...
char hill::my_int_to_char (int i) const
{
...
BOOL hill::ist_erlaubtes_zeichen (char c) const
{
...
char hill::get_fuellzeichen (void) const
{
	return (fuellzeichen);
}

// Funktionen zum Setzen der Dimension
void hill::set_dim (int d)
{
	ASSERT ( (1 <= d) && (d <= HILL_MAX_DIM_GROSS) );
...
int hill::get_dim (void) const
{
	return dim;
}

// Funktionen zum Ermitteln des Moduls
int hill::get_modul (void) const
{
	return modul;
}

// Funktion zum Ermitteln der Laenge der Texte
long hill::get_laenge_plain (void) const
{
	return laenge_plain;
}

long hill::get_laenge_cipher (void) const
{
	return laenge_cipher;
}

// Funktionen zum Setzen des unverschluesselten Textes
void hill::set_plaintext (char *str)
{
	if (plaintext)
	{
		delete[] plaintext;
...
void hill::get_plaintext (char *str) const
{
...
void hill::set_ciphertext (char *str)
{
	if (ciphertext)
	{
		delete[] ciphertext;
...
void hill::get_ciphertext (char *str) const
{
	// hier wird kopiert, Laenge des Feldes muss sein:
...
BOOL hill::set_dec_mat (square_matrix &mat)
{
...
void hill::get_dec_mat (square_matrix *mat) const
{
	// Beide Matrizen muessen die gleiche Dimension haben
	ASSERT (mat->get_dim() == dim);
...
BOOL hill::set_enc_mat (square_matrix &mat)
{
	// Beide Matrizen muessen die gleiche Dimension haben
	ASSERT (mat.get_dim() == dim);
...
void hill::get_enc_mat (square_matrix *mat) const
{
	// Beide Matrizen muessen die gleiche Dimension haben
	ASSERT (mat->get_dim() == dim);
...
int hill::angriff(int min, int max, square_matrix** mat, int rc_angriff_dims[HILL_MAX_DIM_GROSS])
{
	BOOL gefunden = FALSE;
	...
	while ( (testdim <= max) && (! gefunden) )
	{
		...
		square_matrix plainmat(testdim, modul),
					  ciphermat(testdim, modul),
					  hilfsmat(testdim, modul);
		
		square_matrix* ergebnismat;
		ergebnismat = new square_matrix(testdim,modul);
...
void hill::SucheSchluessel (int iRekursionsStufe,	// aktuelle Rekursionsstufe mit
							...
							square_matrix* pErgebnisMat,	// gefundener Schluessel
							square_matrix* pPlainMat,	// Matrix, die aus Klartext aufgebaut wird
							square_matrix* pCipherMat,	// Matrix, die aus verschluesseltem Text aufgebaut wird
							square_matrix* pHilfsMat	// Hilfsmatrix
							)
{
	// Matrix ausgeben
#ifdef _DEBUG
...
void hill::BerechneFaktoren(int i, long lAnzahlBloecke)
{
