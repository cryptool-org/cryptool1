...
private:
	...
	void SucheSchluessel (int,int,long,int*,int*,BOOL*,int*,square_matrix*,
...
public:
	...
	BOOL set_enc_mat (square_matrix&);
	void get_enc_mat (square_matrix*) const;
	
	// Funktionen zum Setzen / Ermitteln der Entschluesselungsmatrix
	BOOL set_dec_mat (square_matrix&);
	void get_dec_mat (square_matrix*) const;
	...
	// Achtung: Die Funktion legt eine square_matrix an, sofern der Returnwert
	...
	int angriff(int,int,square_matrix**,int*);
};
