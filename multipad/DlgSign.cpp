...
CDlgSign::CDlgSign(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgSign::IDD, pParent)
{
	//{{AFX_DATA_INIT(CDlgSign)
	m_edit = _T("");
	m_radio = -1;
	m_SignAlg = -1;
...
void CDlgSign::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlgSign)
...
BEGIN_MESSAGE_MAP(CDlgSign, CDialog)
	//{{AFX_MSG_MAP(CDlgSign)
...
/////////////////////////////////////////////////////////////////////////////
// Behandlungsroutinen für Nachrichten CDlgSign 

BOOL CDlgSign::OnInitDialog() 
{
...
void CDlgSign::InitAsymKeyListBox(unsigned nLocalKeylistType)
{
...
int CDlgSign::FindRow( CString pattern)
{
	int i;
	bool match;
...
void CDlgSign::OnOK() 
{
	...
	else if ( UserKeyId.GetLength() < 1 )
	{
		...
		LoadString(AfxGetInstanceHandle(),IDS_STRING41409,pc_str,STR_LAENGE_STRING_TABLE);
	...
	if ( m_RadioECDSA.GetCheck() || m_RadioECNR.GetCheck() )
	{
		...
		if ( m_CheckSteps.GetCheck() && m_pcRadio.GetCheck() )
		{
			...
			LoadString(AfxGetInstanceHandle(),IDS_STRING41605,pc_str,STR_LAENGE_STRING_TABLE);
			AfxMessageBox(pc_str, MB_ICONEXCLAMATION, 0);
		}
	}
	else
...
void CDlgSign::EnableAllRadioButtons()
{
...
void CDlgSign::UpdateRowSel(int row) 
{
	int i;
	CString Text;
...
void CDlgSign::OnClickList2(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CDlgSign::OnItemclickList2(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
void CDlgSign::OnColumnclickList2(NMHDR* pNMHDR, LRESULT* pResult) 
{
...
int CDlgSign::GetSpecifRow(UINT mask)
{
...
void CDlgSign::OnKeydownList2(NMHDR* pNMHDR, LRESULT* pResult) 
{
