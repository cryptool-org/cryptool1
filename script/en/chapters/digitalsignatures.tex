...
With CT1\index{CT1} you can also generate and check 
...
Also with JCT\index{JCT} (in the default and the algorithm perspektive)
...
CT1\index{CT1} at several places.\\
...
\end{list}
CT2\index{CT2} and JCT\index{JCT} also contain different hash
...
(e.g. \index{SHA-1} SHA--1\footnote{%
  ...
  The current standard \cite{FIPS180} defines four secure hash algorithms
  ...
  With CT1\index{CT1} you can comprehend the birthday attack
  ...
  CT2\index{CT2} contains an MD5 collider.
  }  
...
signatures. The SHA-2\index{SHA-2} family \cite{FIPS180} provides stronger algorithms.
...
\url{https://csrc.nist.gov/groups/ST/hash/sha-3/}\\
    With {\bf CT2}\index{CT2} you can execute and visualize
...
%% BE_8.8.18: Englische Fassung von heise.de nicht mehr erreichbar.
%% Zumindest nicht die bisher angegebene Nummer.
...
(2005) by Reinhard Wobst and J\"urgen Schmidt\footnote{% 
...
Compare: \url{http://en.wikipedia.org/wiki/Digital_signature}.
...
\cite[p.~490]{Schneier1996}, that the operations involved in DSA can be
