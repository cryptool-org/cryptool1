...
\addtocounter{footnote}{0}\footnotetext{%
  ...
  (also see: \href{http://en.wikipedia.org/wiki/Georg\_Christoph\_Lichtenberg}
              {\tt http://en.wikipedia.org/wiki/Georg\_Christoph\_Lichtenberg})
}
...
\caption{Merkle-Hellman procedure (based on knapsack problems)}
...
A. Shamir, \index{Adleman, Leonard} L. Adleman \cite{RSA1978} introduced 
...
Examples'', chapter~\ref{patternsandstructures}.} %or \ref{L-Euler-Function}
\]
...
(see chapter \ref{NoteFactorization}) -- completely without a
...
The Rabin\index{Rabin, Michael O.} public key procedure\index{Rabin!public key procedure}
(crypto procedure~\ref{rabinproc}) has been shown to be equivalent to breaking 
...
\caption{Rabin procedure (based on the factorization problem)}
\label{rabinproc}
\end{cryptoprocedure}
...
Discrete logarithms \index{Discrete logarithm} form the basis for a large
number of algorithms for public-key procedures.
\index{Logarithm problem!discrete}

%--------------------------------------------------------------------
\subsection{The discrete logarithm in $ \Z_p $}
...
and their complexity \cite{4Stinson1995}\index{Stinson 1995}.
\begin{table}[ht]
\begin{center}
...
Index-Calculus) \index{General Number Field Sieve (GNFS)} and took about 17 CPU
...
%--------------------------------------------------------------------
\subsection{ElGamal public key encryption procedure in $ \Z_p^\ast$}
...
asymmetric encryption algorithm, crypto procedure~\ref{elgamalproc}. 
This observation was made by Taher ElGamal.
\begin{cryptoprocedure}
...
\caption{ElGamal (based on the factorization problem)}
\label{elgamalproc}
\end{cryptoprocedure}
...
generatlized ElGamal procedure can be defined (crypto
