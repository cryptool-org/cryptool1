...
Such questions are called {\bf NP}-complete problems (not \index{Runtime!not polynomial NP} deterministically polynomial) that are difficult to 
...
\footnotetext{
Using CrypTool\index{CrypTool} v1.3 you gain practical experience
with the RSA procedure via the menu ``Indiv.Procedures \textbackslash 
RSA Demonstration \textbackslash RSA Cryptosystem''.
}
\index{RSA} \hypertarget{RSAVerfahren}{} 
As early as 1978, R. \index{Rivest Ronald} Rivest, \index{Shamir Adi} A. Shamir,  \index{Adleman Leonard} L. Adleman \cite{RSA1978} introduced 
...
$ e $ be any prime number relative to $ \phi (N) $ \index{Prime number!relative}, i.e. $ \ggt (e,\phi (N))=1. $\index{Gcd}
...
By varying the Diffie-Hellman key agreement protocol \index{Encryption!El Gamal public key} slightly, you can obtain an asymmetric encryption algorithm. This 
