...
(CT1)\index{CrypTool 1} package. The script's goal is to explain some
mathematical topics in more detail, but still in a way which is easy to
understand.
...
%        (see curiouses in \ref{HT-Quaint-curious-Primes-usage}),
  \item the overviews of all functions in
        \hyperlink{appendix-template-overview-CT2}{for CrypTool 2 (CT2)},
        \hyperlink{appendix-function-overview-JCT}{for JCrypTool (JCT)}, and
        \hyperlink{appendix-function-overview-CTO}{for CrypTool-Online (CTO)},
  ...
  \item the chapter about homomorphic encryption
        (see \ref{Chapter_HomomorphicCiphers}).
\end{itemize}

The script was delivered the first time with the CT1 package in
version 1.2.01. Since then it has been expanded and revised in almost
every new version of CT1.
...
\\
\\
Frankfurt and Siegen (Germany), January 2013
\par \vskip + 30pt


\noindent PS:\\
We'd be glad if further authors would show up to add further chapters,
e.g. about the Riemann Zeta function, hash functions, random numbers, or
...
high-score list and a moderated forum. In the meantime more than 3500 active
