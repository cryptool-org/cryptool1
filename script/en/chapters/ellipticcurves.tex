...
(\hyperlink{author_Bartol-Filipovic}{Bartol Filipovic} /
 \hyperlink{author_Matthias-Bueger}{Matthias Bueger} /  % B\"uger
 \hyperlink{author_Bernhard-Esslinger}{Bernhard Esslinger} /
 \hyperlink{author_Roger-Oyono}{Roger Oyono},
 Apr 2000,
 Updates: Dec 2001, Jun 2002, Mar 2003, Nov 2009, Aug 2013, Aug 2016)
...
process keys extending ca. 2000 bit shows that there is a need for
alternatives in the area of asymmetric cryptography. Elliptic-curve
cryptography (ECC) is such an alternative. They are used widely on smartcards.
...
doubles every 18 months)\footnote{%
  Moore's law formulates the empirical observation and the according
  forecast that the number of components or transistors on an integrated circuit
  doubles every two years. It referred originally only to the transistors density
  in an integrated circuit, however not, for example, to the increase in the storage
  density.
  This realization for transistors density in 1965 (with a correction 1975) was
  expressed by Gordon Moore, a co-founder of Intel.
  In recent years, the growth of computing power was even higher than
  the forecasted doubling every 2 years. Future limits are set by the
  transistors size of a few atoms, which could be achieved by 2020.
...
\cite{LeVe2001}).\footnote{%
Further information about key length comparison by Arjen Lenstra und Eric
Verheul, plus more modern evaluations till 2015 can be found in the
interactive website \url{https://www.keylength.com}.
}
...
figure~\ref{ThousandBitMultiplications} (source: J.\ Merkle,
...
\caption{Comparison between signing and verification time for RSA and elliptic curves} 
...
Although there is no need to replace RSA in any application today\footnote{%
Current information about the security of the RSA algorithm can be found in 
chapters \ref{SecurityRSA} and \ref{Chapter_Dlog-FactoringDead}.},
one seriously should take the usage of ECC into consideration\footnote{%
Compare the technical guideline ``Cryptographic Methods: Recommendations and
Keylengths'' of GISA from February 15th, 2016.}.GISA\index{GISA}
More current discussions about the security of ECC can be found in
chapter \ref{Chapter_Dlog-FactoringDead}.
...
In 1993 and 1994\footnote%
{
  1994 the gaps in the first proof have been closed by Wiles and Richard Taylor.
}
, Andrew Wiles\index{Wiles, Andrew} published 
...
also proved the famous second theorem of Fermat\index{Fermat!last theorem}.
Mathematicians had spent centuries (Fermat\index{Fermat, Pierre} lived from
1601 to 1665) trying to find a
...
(written in the borders of a book from Diophantus):
...
procedures based on elliptic curves are based on the difficulty of the problem
known as elliptic curve discrete logarithm\index{logarithm problem!discrete}.
The factorization of natural composite numbers $n$ uses the fact that a large
number of elliptic curves can be generated for $n$.%; however, these curves
%then are not groups for composite $n$.
...
\index{field} {\em fields}.\footnote{%
  A didactically well prepared introduction into Elliptic Curves can be found
  in \cite{Schulz2015}.\index{RSA \& Co. at school}
}
...
If in addition it applies $ab = ba$ (commutative law) for all $a, b$ in $G$, then we call
the group an {\em Abelian} group.
...
${\mathbb Z}$. ${\mathbb Z}$ has an infinite number of elements:
...
For our purpose, so-called {\em finite} groups play an important role. This means
that there exists a set $\mathcal{M}$ with a fixed number of elements and an
operation $+$ such that the above conditions are fulfilled. One example is the
group ${\mathbb Z}_n = \{0, 1, 2, 3, \cdots, n-1\}$ of the remainders of the
division by $n \in {\mathbb N}$, and the operation is an addition mod $n$.
So, for example, $a$ and $b$ in ${\mathbb Z}_n$ are subject to the operation
$a+b \;{\rm mod~} n$.
...
In cryptography elliptic curves are a useful tool.\footnote{%
  We write ``elliptic-curve cryptography'' with a hyphen, according to ``public-key cryptography''.
  Sadly, this isn't used consistently in the literature.
}

Such curves are described by some equation. A detailed analysis has shown that curves
of the form\footnote{%
  This curve is given by the zeros of a {\it polynomial}\index{polynomial}
  $F$ of degree three in three variables. In general, expressions of the form
  ...
  with $i_1,\dots,i_n \in \mathbb{N}$
  with coefficients $a_{i_1\dots i_n}\in K$ are called polynomials in $n$ variables
  $x_1,\dots,x_n$ with underlying field $K$, if ${\rm deg\,} P:=\max\{i_1+\dots +i_n:
  a_{i_1\dots i_n}\ne 0\}$ is finite, i.e. the sum has only finitely many non-zero
  terms (monomials). The sum of the exponents of the variables of each term of the
  sum is at most $3$, at least one term of the sum has a single variable with $3$ as
  value of the according exponent.
}
...
\caption{Example of an elliptic curve with the real numbers as underlying field} 
...
\url{https://www.certicom.com/ecc-tutorial} (Date of last update is unknown).\\
...
\footnote{John M. Pollard \index{Pollard, John M.} was involved
...
based on elliptic curves (see \cite{Lenstra1987}).
}
...
\footnote{%
The dialog box, which appears in CT1\index{CT1} after clicking the menu
...
Step-by-step point addition on elliptic curves is visualized in CT1\index{CT1}
and in JCT\index{JCT}.%
\footnote{%
...
In SageMath elliptic curves are very well described at:%
\footnote{%
According SageMath samples can be found e.g. also in the "Elliptic Curve Cryptography
(ECC) Tutorial"\\
...
\noindent SageMath notebooks are running after a logon within a browser%
\footnote{%
If you installed SageMath on your own (Unix) server, you first have to
enter at the command line the command \verb#notebook()#.

}${}^,$\footnote{%
The \hyperlink{ec:Web-Link:Sage_Massierer}{ECC notebook} of Massierer
needs the KASH3 library: Therefore e.g.\ with SageMath 4.2.1%TODO-Version
the package ``kash3-2008-07-31.spkg'' has to be installed (command \verb#sage -i#).
}.


%--------------
The \hyperlink{ec:Web-Link:Sage_Massierer}{ECC notebook}
\index{elliptic curve!ECC notebook}\index{Massierer, Maike}
created in 2008 by Massierer\footnote{%
Instructions to use an interactive SageMath notebook\index{SageMath!instructions
interactive notebook}:
Update for the new SageMathCloud xxxxxxxxxxxxx\\
% TODOTODO

  \noindent\hangindent=6pt\makebox[6pt][l]{-}%
  Some SageMath servers are publicly available
  %like \url{http://sage.mathematik.uni-siegen.de:8000} or
  %\url{http://www.sagenb.org/}
  and offer running samples as ``Published Worksheets'',
...
% 
%  TODO --> SageMathCloud xxxxxxxxxxx
%  \noindent\hangindent=6pt\makebox[6pt][l]{-}%
...
%  --> Now
%  https://cloud.sagemath.com/projects/19575ea0-317e-402b-be57-368d04c113db/files/pub/1101-1201/1126.sagews
...
%  disappear and leave you with a nice layout of the worksheet.
  ...
  the TIFF fonts. This font installation has to be done at every PC.
%
%  \noindent\hangindent=6pt\makebox[6pt][l]{-}%
...
consists of 8 parts (title page with contents plus 7 chapters)
...
``Irrelevant patents on elliptic-curve cryptography''. Retrieved 2016-07-14.
}.
...
\noindent All links have been confirmed at July 14, 2016.
...
\begin{enumerate}
   ...
   \item Interactive introduction to elliptic curves and elliptic curve
        cryptography with SageMath\index{SageMath} 
   ...
   \item Overview of Elliptic Curve Cryptosystems,\\
        ...
        M.J.B. Robshaw and Yiqun Lisa Yin.\\
        RSA Laboratories\index{RSA Laboratories},
        \begin{sloppypar}
   ...
   \item Working group IEEE P1363,\\
		IEEE P1363 is an IEEE standardization project for public-key cryptography.\\
		Elliptic curves were part of so called established or traditional public-key documents. The project began in 1993. The group's original website \textit{http://grouper.ieee.org/groups/1363} is no more accessible (status of 2018-08-08).
   ...
   \item \hypertarget{Lenstra2}{}
	...
	elliptic curves as well as links to other web pages.\\
        \url{https://members.loria.fr/PZimmermann/records/ecmnet.html}

   \item BSI TR-02102-1,\\
	Technical Guideline for Cryptographic Methods: Recommendations and
        Keylengths by GISA\index{GISA} (German Information Security Agency),\\
	February 15th, 2016. (German only)
        ...
        \begin{sloppypar}
        \url{https://www.bsi.bund.de/EN/Topics/ElectrIDDocuments/TRandSecurProfiles/technicalGuidelinesSeite.html}\\
