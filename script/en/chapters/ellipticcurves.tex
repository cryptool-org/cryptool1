...
Koblitz\index{Koblitz Neal} and Victor Miller\index{Miller Victor} independently suggested using elliptic curves in
...
then groups for composite $n$. More information about this can be found in
section \ref{faktell}.
...
${\bf E}$ has precisely $p$ elements) and the {\em super-singular} curves
...
The secret key $s$ of the signature generator is a (random\index{Random}) whole number $s$ in
...
The biggest compound number currently factorised\index{Factorisation!factoring records} with elliptic curves is 
