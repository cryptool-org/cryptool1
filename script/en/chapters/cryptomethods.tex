...
June 2005, July 2007, Jan. 2010, Nov. 2012)
...
For further explanations we use the notation from the following Figure \ref{Generic-Notations-when-Encrypting}:
\begin{figure}[ht]
\begin{center}
...
Example: Currently we know that RSA is at most as difficult as factorization,
but we cannot prove that its exactly as difficult as factorization. So RSA
has no proven minimum security. Or in other words: We cannot prove, that if
RSA (the cryptosystem) is broken, that then factorization (the hard
...
considered (practically) secure -- from a pure algorithm's point of view.%\par \vskip + 3pt
...
{Symmetric encryption\footnotemark}
  \footnotetext{%
    With CrypTool 1 ({\bf CT1})\index{CrypTool 1} you can execute the following modern
    ...
    With CrypTool 2 ({\bf CT2})\index{CrypTool 2} you can execute the following modern
    symmetric encryption algorithms (using in the Startcenter {\bf Templates
    \textbackslash{} Cryptography \textbackslash{} Modern \textbackslash{}
    Symmetric}):\\
    AES, DES, PRESENT, RC2, RC4, SDES, TEA, Triple-DES, Twofish.\\
    In JCrypTool ({\bf JCT})\index{JCrypTool} you can execute the following modern
    symmetric encryption algorithms:\\ 
...
\addtocounter{footnote}{0}\footnotetext{Daniel Suarez, ``Freedom'', Dutton Adult, 2010,
                          Chapter 5, ``Getting with the Program'', p. 63, Price.}
...
More details and further references about the AES algorithms and the AES candidates
of the last round can be found i.e. within the online help of
...
\footnote{%
    With CrypTool 1\index{CrypTool 1} you can also perform brute-force attacks
    ...
    With CrypTool 2\index{CrypTool 2} you can also perform brute-force attacks
    (using the templates under {\bf Cryptanalysis \textbackslash{} Modern}).
    Highly powerful is the KeySearcher component, which can be used to distribute
    the calculations to many different computers.
}.
...
They offer prizes for those who managed to decipher ciphertexts, encrypted with different algorithms and different key lengths, and to unveil the symmetric key (under controlled conditions). So theoretical estimates can be confirmed.
...
{Asymmetric encryption\footnotemark}
  \footnotetext{%
    With CrypTool 1\index{CrypTool 1} you can execute RSA encryption
    ...
    With CrypTool 2\index{CrypTool 2} you can also perform asymmetic methods
    (using the templates under {\bf Cryptography \textbackslash{} Modern}).\\
    JCrypTool\index{JCrypTool} offers asymmetic methods like RSA both within
...
\footnotetext{%
   ...
   JCrypTool\index{JCrypTool} offers hybrid methods like ECIES within
   the Functional Perspective under {\bf Algorithms \textbackslash{} Hybrid Ciphers}.
}
...
sender randomly\footnote{%
   ...
   generators. Only the integrated Secude generator involves a "pure" random source.\\
   JCrypTool\index{JCrypTool} offers {\bf pseudo} random number generators both
   within the Default Perspective in the menu {\bf Algorithms \textbackslash{}
   Random Number Generator} as well as within the Functional Perspective.
}\index{Random}
