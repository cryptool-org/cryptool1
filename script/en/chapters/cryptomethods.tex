...
This session key is encrypted using the asymmetric procedure, and then
...
encryption and decryption by hand and gain a better understanding of how the algorithms
work in detail. One is e.g. S-AES (Simplified-AES) by Prof. Ed Schaefer and his students%
\footnote{
    \url{http://math.scu.edu/~eschaefe/getfile.pdf} \\
    \url{http://www.scu.edu/cas/research/cryptography.cfm}
}. Another one is Mini-AES (see chapter~\ref{CM_Sage_Mini-AES} ``\nameref{CM_Sage_Mini-AES}'').
...
The following Sage code from the release note of Sage 4.1\\
\url{http://mvngu.wordpress.com/2009/07/12/sage-4-1-released/}\\
calls the implementation of the Mini Advanced Encryption Standard.

The Sage module \texttt{crypto/block\_cipher/miniaes.py} supports Mini-AES to allow students
to explore the working of a modern block cipher. This is a simplified variant of
the Advanced Encryption Standard (AES) to be used for cryptography education.
Mini-AES is described at \cite{cm:Phan2002}.
