% test am 29.06.18 von doris -- ist angekommen.
% $Id$
% !Mode:: "TeX:DE"    % Setting document mode and submode for WinEdt
% ............................................................................
%        MAIN- or STARTING-Datei f¸r das CrypTool-Buch-Dokument
%
...
%                - der Prozess; Genitiv: des Prozesses (duden.de/rechtschreibung/Prozess)
%
% Rechtschreibung dt: - eine abelsche Gruppe    --> "abelsche" klein.
%                     - eine Boolesche Funktion --> bisher immer groﬂ [passt das zu Abel?]
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%
% Housekeeping:
...
%                  [ Abstandsm‰ﬂig etwas enger w‰re: "CrypTool\,2" ]
%                  Analog f¸r CrypTool 1  done (d+E).
...
% be_2018-05-16: - cover page now with 2018 (got rid of lower resolution for drafts)
%                - TeXlive 2018: commandline-make on Windows and WinEdt both worked well.
%                - removed babel warnings "empty language", added language
%                  in each bibliographies record.
%                - cleaned up the comments in CT-Book-xx.txt and reference.bib.
%                - small typos, dead links, ...
%
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%
...
% 2) Black boxes kamen auch nach Einf¸gen der Referenzen:
...
%    - ...
% 2) bibtex durch biblatex ersetzen ?
%    D¸rfte aber viele Problem machen, denn laut Doc zu v3.4 in
...
% 3) Maybe switch to biber to have more possibilities for comments in references.bib.
%    Biber respects %, whereas BibTeX does not.
%  (https://tex.stackexchange.com/questions/93972/comment-out-sections-of-text-in-bib-file)
%    - Used the workaround "_language =" to see where US-English is meant according to
%      (https://tex.stackexchange.com/questions/21709/comments-in-bibtex)
%    - However, the real solution should include to specify USenglish explicitely.
% 4) Added language= in each bibliography entry, because of bibtex warning "empty language".
%    Despite the internet recommendations this caused another warning then
%    from babel: Package babel Error: Unknown language `USenglish'.
%    Same with "german"  -->  changed to ngerman in referencs.bib.
%    Changed `USenglish' to `english' --> WinEdt works; make still has
%    wrong page numbers in the index in the 1st run. Needed to delete
%    the pdf and run the commandline make and 2nd time. Helped 14.5.18,
%    but not at 16.5. ==> Output from WinEdt more reliable at the moment.
% 5) Nice-to-have: Berechnte Zeilenzahl in Tabellen-Unterschrift bringen
%%   beg-VERSUCH-TRY_FP: just a try, didn't succeed----------------------
%    Tried to write the row number from the last row in table 3.2 into the header
%    of the table. As each call make it to increment, I need to do calculations.
%    ==> Probably using Python is much better than the package fp.
%    http://anamorphose.de/rechnen.pdf
%    https://stackoverflow.com/questions/1219121/is-there-a-calculator-with-latex-syntax  --> SageMath
%    https://tex.stackexchange.com/questions/154224/fp-calculation-variable
%    https://tex.stackexchange.com/questions/30081/how-can-i-sum-two-values-and-store-the-result-in-other-variable   %    and  http://ctan.org/pkg/fp
%%%    \FPeval{\result}{clip(5+6)}%
%%%    $5+6=\result$
%%%    \FPeval{\result}{round(2+3/5*pi,5)}%
%%%    $2+3/5\times\pi=\result$
%% \usepackage[nomessages]{fp}
%%   end-VERSUCH-TRY_FP: -------------------------
% 6) Nach KOMA-Script konvertieren.
%    Ziel: HTML-Seite und epub3-Dokument zus‰tzlich generieren aus den TeX-Sourcen.
% 7) Warum so viele Warnungen von der Art:
%     - pdfTeX warning (ext4): destination with the same identifier
%      (name{cite.Wang2005b}) has been already used, duplicate ignored
%     - LaTeX Warning: Reference `s:appendix-using-sage' on page ii
%       undefined on input line 22.
%     [Momentan, Mai 2018, haben die B¸cher 552 und 568 Seiten insgesamt;
%     und im E werden 439 Warnings und 342 Bad Boxes gemeldet; im D: 438/358.]
...
\documentclass[a4paper,11pt,oneside,english,ngerman]{book}  % the order ngerman-english doesn't seem to matter.
\overfullrule=5pt % uncomment to mark overfull boxes
\usepackage[latin1]{inputenc} % Umlaute  %%BERM2: Added for bitciphers.tex (still necessary?)
%%% \usepackage[utf8]{inputenc}   % Bessere Alternative? --> Doris
...
% Statt T1 als Package zu inkludieren, nur die wirklich benˆtigten Sonderzeichen definiert.
...
%%%     F¸gte $-Klammer drumherum, was wahrscheinlich eh korrekter. 5.8.16

%% Declare all languages needed in the document. The last in the list should be the main one, but this didn't really work. However, after specifying with "main=" in English in \usepackage[ngerman,main=english]{babel} it worked as expected (before in CT-book-en.pdf \today on 1st page had German format, and \listfigurename and \listtablename were in the wrong language: Abbildungsverzeichnis instead of List of Figures).
%% Package babel only necessary, because package babelbib was used.
%% See https://tug.org/pracjourn/2007-1/gregorio/gregorio.pdf
%% https://tex.stackexchange.com/questions/82993/how-to-change-the-name-of-document-elements-like-figure-contents-bibliogr    +++
\usepackage[ngerman]{babel}  % ngerman (f¸r neue dt. Rechtschreibung) geht nur, wenn auch bei documentclass gesetzt.

%% If language is specified in each bib entry and "[fixlanguage]",
%% then \selectbiblanguage{english} should not be needed.
%% Language in \selectlanguage{english} must previously be loaded by babel as option.
%% Needed in addition since usepackage babel loaded more than one language. Important,
%% that it's located after \begin{document}!  (Would not be needed, if "main=english" worked.)
\usepackage[fixlanguage]{babelbib}
\selectbiblanguage{ngerman}
%% Wirkung im D in Bibliography: editors -> Herausgeber,
%%         im D: bei edition aus "1" eine "1." (ohne babelbib "1" unver‰ndert).
%%         im E: Leider macht es aus Mar nicht M‰rz  --> Doris (minor importance)
%% \bibliographystyle{babplain}
%%% \bibliography{../de/references.bib}  %% Evtl. hier EINMAL setzen, statt in jedem \putbib[../de/references]  -->  erledigt per \defaultbibliography{../de/references}


% \usepackage{geometry} %%%% 1.1.17: Added to make right border smaller for page with menutree only // Wieder raus, da danach li. Rand zu groﬂ! M¸sste wohl alles gleich zu Beginn hier setzen.
...
\usepackage{morewrites}% Neu wegen "message {No room for a new \string #" nach Hinzuf¸gen von \lostof{opensslcode}
...
\usepackage{fancyvrb}
\usepackage{makeidx}  % Alternativ evtl. \usepackage{imakeidx} (bemerkte keinen Unterschied) --> Alphabetheader wahrscheinlich mˆglich!
...
%                   %BE_2016: Wollte, dass topsep, partopsep, itemsep und nolistsep bekannt.
% http://tex.stackexchange.com/questions/119919/no-spacing-between-enumerated-items-with-usepackageenumerate
%      (enth‰lt Beispiel f¸r Multiple Choice Quizz)
...
% Formatierung der Fuﬂnoten
\usepackage[flushmargin]{footmisc}
\usepackage{fullpage}

% Rahmen um Boxen, die auch mehrzeilig sein kˆnnen (siehe Euler-Phi-Def.)
% \usepackage{framed}  %% Nicht gebraucht, da man daf¸r auch \parbox innerhalb von \fbox aufrufen kann.
...
\do\x\do\y\do\z}
% Trennung in langen URLs nach jedem Kleinbuchstaben mˆglich (braucht manchmal zus‰tzlich sloopypar).
...
%     oder
%        \usepackage{microtype} %Sorgt f¸r bessere Platzausnutzung der \hbox bei Umbr¸chen
%        \setlength{\emergencystretch}{1em} %Sorgt f¸r bessere Platzausnutzung der \hbox bei Umbr¸chen
%
% \usepackage{breakurl}  %BE_2016Jul: War nicht nˆtig!
...
% den "\url{}"-Befehl in einem BibTeX-File verwenden und erh‰lt einen
% Zeilenumbruch in der Url, ohne dass die Mˆglichkeit in PDF-Dokumenten
% verloren geht, den Link direkt auszuf¸hren.


% Literaturverzeichnis kapitelweise
%%% \usepackage[labelstoglobalaux]{bibunits} % ohne labelstoglobalaux scheint man thebibilography und bibtex nicht mischen zu kˆnnen
...
\defaultbibliography{references}  % One common reference.bib for de and en in script/de
%%%NEWNEW \defaultbibliographystyle{abbrv}
...
\usepackage{eurosym}%Necessary for Ä symbol in chap 8.3.1 bitcipers.tex (not in en version)
...
% !!! BEACHTE: Several problems can be avoided, if hyperref is the last package loaded !!!!!!!!!!!!
%% https://www.apfeltalk.de/community/threads/latex-hyperref-ja-aber-bitte-ohne-rahmen.82217/
%16xxxxxxxxxxxxxx In dlog.tex war: \usepackage{hyperref,cite,url}
\usepackage[colorlinks=true,linkcolor=blue,citecolor=blue,urlcolor=black,
...
% With  "\newfloat{opensslcode}{!ht}{loc}[chapter]"  both directories (sagecode and opensslcode)
% contained all entries --> It's important to add a new extension name
% (file where the temp info is written to) per list type!!!
...
% --> Sp‰ter: Use the KOMA-script class or the memoir class --> Doris
...
% 	       Das CrypTool-Buch:\\[20pt]
	...
	%{\large 12. Auflage}
	 {\large 12. Auflage --  {\bf Draft-Version} (\settimeformat{hhmmsstime}\currenttime)}
        \vspace{150pt}
	% War der \vspace-Wert zu groﬂ (z.B. 180), kam nach dem Cover eine
	...
	% rˆmische I ?  D.h. er begann zweimal von vorne!
}
...
% \includepdf[pages=1]{figures/coverv1_de_draft.pdf}  % 100dpi (kein Draftbalken mehr, nur geringere Auflˆsung)
% \includepdf[pages=1]{figures/coverv1_de.pdf}        % 300dpi (wieder pdf statt png)
\includepdf[pages=1]{figures/coverv1_de.pdf}        % ab 2017 nur eine Auflˆsung und kein Querbalken
...
\frontmatter % Ab hier beginnt Seitennummerierung mit rˆmischen Ziffern
\maketitle


% Vergleichbar zur Nutzung von Texten aus der Wikipedia (=GNU Free Documentation Licence, GFDL)
\begin{quote}
Dies ist ein freies Dokument, d.h. Inhalte des Dokuments kˆnnen kopiert und
...
nennen. Selbstverst‰ndlich darf aus dem CrypTool-Buch, genau wie aus jedem
...
\input{chapters/overviewcontent.tex} % ‹berblick ¸ber den Inhalt des CT-Buches
...
% Die Datei references.bib wird von CTB-de und CTB-en genutzt und steht genau einmal (!) in trunk\script\de (gilt also auch f¸r trunk\script\en).
%%% Ziele f¸r die Bibliography-Ausgaben:
%%% 1) Am Buch-Ende zwei Gesamt-Bibliography-Ausgaben, mit unterschiedlichem Style (!).
%%% 2) ‹berall im Text sollen die automatisch erzeugten alpha-Referenzen angezeigt werden.
%%% 3) Am Kapitel-Ende soll die bibliography nur die im Kapitel erw‰hnten Referenzen auff¸hren.
%%%    Das geht nur, wenn man das \begin{bibunit}[plain] und \end{bibunit} jeweils auch dort
%%%    am Tex-File-Beginn und -Ende hat. Macht man das hier im main-File um jede Datei herum,
%%%    wird es un¸bersichtlich.
%%%    Alternativ h‰tte ich es wohl auch mit biblatex probieren kˆnnen.
...
%
%%%x  \bibliographyunit[\chapter] % \bibliographyunit mit [chapter] leite unit-weise ein; ohne Arg. wieder normal.
%
% Stern hier vor der {...}, da dies nur den Style in der local units festlegt und nur daf¸r Infos generiert.
...
% ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%xy%%_17-01-19: Introduced LaTeX var CTBChapName to have a specific headline for the chapter bibliography
...
% \bibliographyunit  % Use \bibliographyunit with no arguments to deactivate
                     % bibliographyunits. By default it is deactivated.
             % W‰re nur nˆtig, wenn ich vor den Includes \bibliographyunit[\chapter] benutzt h‰tte.
...
%                       L‰uft mir damit eine Registerzahl ¸ber? Welche?
...
%%% \xxaddcontentsline{toc}{chapter}{Bibliography of All Chapters}  % section, chapter oder * legen
%%%     nur die Gliederungsebene fest f¸r die Ausgabe im Inhaltsverzeichnis.
...
%% Die Nummerierung in der 2. Gesamt-Bibliography hier gilt also nur als Aufz‰hlung.

%%% TESTTEST Die erste der beiden Gesamt-Bibliotheken kann man w‰hrend der Entwicklung auch
...
%% Style plain: Gesamt-Bibliography zeigt Zahlen und ist unsortiert nach Autor/Jahr.
\begin{bibunit}[babplain] %% Style not just plain! %% \begin{bibunit}[plain]
\nocite{*}
	\renewcommand{\bibname}{Literaturverzeichnis ¸ber alle Kapitel (nummeriert by babplain)}
\putbib[../de/references]
	\addcontentsline{toc}{chapter}{Literaturverzeichnis ¸ber alle Kapitel (nummeriert)}
\end{bibunit}

%% alpha: Gesamt-Bibliography zeigt Autorenk¸rzel und ist unsortiert nach Autor/Jahr.
%%        Da dies der letzte Bibliography-Aufruf ist, stehen im Quelltext auch die Autorenk¸rzel.
\begin{bibunit}[babalpha] %% Style not just alpha! %% \begin{bibunit}[alpha]
...
\renewcommand{\bibname}{Literaturverzeichnis ¸ber alle Kapitel (sortiert by babalpha)}
\putbib[../de/references]
\addcontentsline{toc}{chapter}{Literaturverzeichnis ¸ber alle Kapitel (sortiert nach AutorJahr)}
\end{bibunit}
...
%     http://tex.stackexchange.com/questions/186019/makeindex-and-see-subentry
  ...
  \index{Schl¸sselaustausch!Diffie-Hellman|see{Diffie-Hellman}}
  \index{ECC|see{elliptische Kurve}}
  \index{Triple-DES|see{DES, Triple-DES}}

  \index{Fermat!groﬂer Satz|see{Fermat, letzter Satz}}
  \index{Fermat!zweiter Satz|see{Fermat, letzter Satz}}
  \index{groﬂer Satz|see{Fermat, letzter Satz}}
  ...
  % \index{MysteryTwister C3|see{Krypto-Wettbewerb}}  Raus, da 2 Verweise nicht schˆn, und vor allem
  %            werden sie alphabetisch geordnet und nach "siehe Krypto-Wettbewerb" ist eine Blackbox.
  \index{Challenge|see{Krypto-Wettbewerb}}
  % \index{Cipher-Challenge|see{Krypto-Wettbewerb}}  Raus, f¸hrt trotz RaggedRight zu Blackbox.
  ...
  \index{r¸ckgekoppeltes Schieberegister|see{Schieberegister}}
