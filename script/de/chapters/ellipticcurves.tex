...
(Filipovics B. / Büger M. / Esslinger B. / Oyono R., April 2000,
 Updates: Dez. 2001, Juni 2002, März 2003, November 2009)
...
\footnote{empirische Erkenntnis von Gordon Moore, Mitbegründer von Intel,
...
\cite{ec:Lenstra1999} (Quelle: Arjen Lenstra und Eric Verheul:
...
\caption{Prognose für die Entwicklung der als sicher betrachteten
  Schlüssellängen für RSA und Elliptische Kurven\vspace{1ex}} 
\label{RSAKeylength}
\end{center}
\vskip -30 pt
\end{figure}
...
\caption{Gegenüberstellung des Aufwands der Operationen Signieren und
         Verifizieren bei RSA und Elliptischen Kurven} 
...
\index{Gruppen} {\em Gruppen} und \index{Körper} {\em Körper}.
...
\begin{itemize}
   \item Eine Gruppe ist eine nichtleere Menge $G$ mit einer Verknüpfung ``$\cdot$''. Die Menge $G$ ist unter der
...
In der Kryptographie betrachten wir Elliptische Kurven. Solche Kurven ergeben sich als Lösung"-en einer Gleichung der Form\footnote{Die hier verwendete Kurve erhält man als Nullstellen des {\it Polynoms}\index{Polynom} $F$ vom Grad drei in drei Variablen. Dabei bezeichnet man allgemein Ausdrücke der Form
...
$$
\end{enumerate}
Hieraus folgt insbesondere für $P=(x,y)\in E$, dass gilt $-P=(x,x+y)$. (Beachte: $-(-P)=(x,x+(x+y))=(x,2x+y)=(x,y)$, da der zugrunde liegende Körper Charakteristik $2$ hat.)\footnote{Eine Animation der Punktaddition auf Elliptischen Kurven 
...
Dies wird als das \glqq Diskrete Logarithmus Problem über Elliptischen Kurven\grqq\ bezeichnet
(auch ECDLP\index{ECDLP} - Elliptic Curve Discrete Logarithm Problem - abgekürzt).
...
Um einen ``Man-in-the-Middle''-Angriff zu verhindern, kann man auch hier wie schon in Kapitel \ref{Impersonalisierungsattacke} beschrieben, die übertragenen Werte $G,q,R_A,R_B$ digital signieren.
...
Im Jahr 1987 stellte H.W. Lenstra\index{Lenstra 1987}
einen Faktorisierungsalgorithmus vor, der auf Elliptischen Kurven basiert 
(siehe \cite{ec:Lenstra1987}). Die aktuell größte mit Elliptischen Kurven 
...
Im Folgenden wird aufgezeigt, welche Funktionalität dazu in CrypTool und in Sage vorhanden ist.

% -----------------------------------------------------------------------------
\subsection{CrypTool}

CrypTool enthält Elliptische Kurven, um digitale Signaturen zu erzeugen%
\footnote{%
Die Dialogbox, die in CrypTool\index{CrypTool} nach dem Menü 
...
\footnote{%
Sage-Beispiele dazu finden sich z.B. unter den \glqq Published Worksheets\grqq~auf
\url{http://www.sagenb.org/pub/}:\\
- über Elliptic Curve: \url{http://www.sagenb.org/home/pub/606/} \\
- über Elliptic Curve El Gamal: \url{http://www.sagenb.org/home/pub/104/}, oder im
...
\noindent Ein ausführliches, interaktives ECC-Tutorial, genannt
\hyperlink{ec:Web-Link:Sage_Massierer}{ECC-Notebook},
gibt es von Maike Massierer.
Diese Einführung in die Elliptische-Kurven-Kryptographie (ECC)
ist als Sage-Notebook aufgebaut.

Sage-Notebooks werden im Browser nach einem Logon-Vorgang aufgerufen%
\footnote{%
Hat man Sage auf einem eigenen (Unix-)Server installiert, muss man auf der Sage-Kommandozeile 
erst den Befehl \verb#notebook()# aufrufen.
}${}^,$\footnote{%
Das ECC-Notebook von Maike Massierer benötigt die KASH3-Bibliothek: Deshalb muss (z.B. für Sage 4.2.1)
das Package \glqq kash3-2008-07-31.spkg\grqq~installiert worden sein (Befehl \verb#sage -i#).
}.

Das ECC-Notebook\index{Elliptische Kurven!ECC-Notebook}\index{Massierer, Maike} von
Massierer\footnote{%
Loggt man sich auf einem fremden, öffentlichen Server wie
\url{http://sage.mathematik.uni-siegen.de:8000/home/pub/45/} oder
\url{http://www.sagenb.org/} ein, und liegen dort die Worksheets als
\glqq Published Worksheets\grqq~schon vor, kann man folgendermaßen vorgehen:\\
- Starten des Sage-Notebooks in einem Browser.

  \noindent\hangindent=6pt\makebox[6pt][l]{-}%
  Klickt man auf \glqq Published\grqq~in der oberen rechten Ecke, sieht man die 8 Worksheets.

  \noindent\hangindent=6pt\makebox[6pt][l]{-}%
  Manchmal ist es nötig, dass man zu Beginn in der linken oberen Ecke
  \verb#Action -> Evaluate all# klickt.

  \noindent\hangindent=6pt\makebox[6pt][l]{-}%
  Manchmal enthält das Fenster viele (blaue) Fehlermeldungen. Das kann man normalerweise
  schnell lösen, indem man den \glqq \%hide\grqq-String anklickt: Danach sieht
  man den Code hinter der Grafik. Mit Shift-Enter kann man die Grafik neu erzeugen.

  \noindent\hangindent=6pt\makebox[6pt][l]{-}%
  Dieses ECC-Tutorial benutzt die Fonts jsMath. Für eine bessere Darstellung sollten
  diese in Ihrem Browser installiert sein.
  ...
  TIFF-Fonts.
  Leider muss die Installation der Fonts an jedem PC einzeln erfolgen.

  \noindent\hangindent=6pt\makebox[6pt][l]{-}%
  Wenn Sie ein Worksheet ändern wollen, sollten Sie Ihre eigene lokale Kopie
  (unter Linux im HOME-Verzeichnis) erstellen (Klick auf \glqq Edit a copy\grqq~in
...
Der Erfolg dieser Standardisierungsbemühungen ist heute noch nicht absehbar.
...
Bereits heute werden Elliptische Kurven in der Praxis eingesetzt.
...
der Schlüssellänge (Speicherplatz) eine entscheidende Bedeutung zukommt.
...
\begin{enumerate}
   ...
   \item Umfassende interaktive Einführung in elliptische Kurven und
        Elliptische-Kurven-Krypto"-graphie (ECC) mit Sage\index{Sage}
      ...
      {\texttt{http://sage.mathematik.uni-siegen.de:8000/home/pub/45/}} \\
        ...
        Version 1.01, Oktober 2008
        % --> seit April 2009 ohne HTTPS auf Siegener Server (mit SAGE 3.4)

  \item Online-Tutorial über Elliptische Kurven der Firma Certicom\index{Certicom},\\
  ...
  \item Schlüssellängen-Vergleich von Arjen Lenstra und Eric Verheul,\\
        \href{http://cryptosavvy.com/table.htm}
	{\texttt{http://cryptosavvy.com/table.htm}}
\end{enumerate}
