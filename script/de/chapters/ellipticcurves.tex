...
(Filipovics B. / Esslinger B. / Oyono R. / B"uger M., April 2000, Updates: Dez. 2001, Juni 2002)

\subsection{Elliptische Kurven --- ein effizienter Ersatz f"ur RSA?}

In der Fianzwirtschaft sind Sicherheit und Effizienz von Daten"ubertragungen von gro"ser
Wichtigkeit. Viele Anwedungen verwenden den RSA-Algorithmus als asymmetrisches Verschl"usselungsverfahren.
...
geführt, dass die ben"otigten Schl"ussell"angen mehrfach angehoben werden mussten. Aktuell werden
standardm"a"sig noch 1024-bit-Schl"ussel verwendet. Das BSI (Bundesamt f"ur Sicheheit in der
Informationstechnologie) empfiehlt jedoch sp"atestens ab dem Jahr 2006 eine Mindestl"ange von 2048 bit.
Bedenkt man, dass die meisten Chips auf Smart Cards nicht in der Lange sind, l"angere Schl"ussel als
1024 bit zu verarbeiten, so zeigt dies den Bedarf f"ur Alternativen zum RSA. Elliptische Kurven
k"onnen eine solche Alternative bieten.

Die Effizienz eines kryptographischen Algorithmus h"angt wesentlich von der ben"otigten Schl"ussell"ange
...
dass die ben"otigten Schl"ussell"angen erheblich k"urzer sind. Setzt man den Trend, dass sich die
Leistung der verf"ugbaren Rechner im Schnitt alle 18 Monate verdoppelt (Gesetz von Moore), in die Zukunft
fort, so muss man von einer Entwicklung der ben"otigten Schl"ussell"angen wie in der folgenden Graphik ausgehen.

\vskip -20 pt
% -> Figur 1
\begin{center}
\begin{figure}[h]
\includegraphics[scale=0.5]{figures/RSAKeylength}
% \caption{} 
\end{figure}
\end{center}
\vskip -30 pt
...
Betrachtet man ferner den Rechenaufwand, die f"ur die Erstellung einer digitalen Signatur notwendig ist,
so stellt man fest, dass auf Elliptischen Kurven basierende Verfahren im Vergleich zu RSA nur gut ein
Zehntel des Rechenaufwandes ben"otigen. Betrachtet man die f"ur eine Verifikation durchzuf"uhrenden
Rechenschritte, dreht sich dieses Bild jedoch zu Gunsten des RSA um. Der Grund liegt darin, dass es bei
Verwendung des RSA m"oglich ist, einen sehr kurzen "offentlichen Schl"ussel zu w"ahlen, solange der
private Schl"ussel nur hinreichend lang ist.

% -> Figur 2
\begin{center}
\begin{figure}[h]
\includegraphics[scale=0.5]{figures/ThousandBitMultiplications}
% \caption{} 
\end{figure}
\end{center}
\vskip -20 pt

Da bei Smart Card basierten Systemen stets der lange private Schl"ussel auf der Karte gespeichert werden
muss und die digitale Signatur, nicht aber die Verifikation auf der Karte stattfindet, treten hier deutlich
die Vorteile Elliptischer Kurven zu Tage. Unter Effizientgesichtspunkten sind Verfahren, die auf Elliptischen
Kurven basieren, daher RSA--Anwendungen vorzuziehen.
\par
\smallskip
Das gr"o"sste Problem bei der Implementierung von Verfahren, die auf Elliptischen Kurven beruhen, ist bislang die
mangelnde Standardisierung. Es gibt nur eine RSA-Implementierung, aber viele Arten, Elliptische Kurven einzusetzen.
...
sind. Um mit einer beliebige auf Elliptischen Kurven basierenden Anwendung kommunizieren zu k"onnen, m"ussete man
eine Vielzahl von Implementierungen vorhalten, was den Effizientvorteil gegen"uber der Verwendung von RSA
zunichte macht.

Um dies zu vermeiden, bem"uhen sich internationale Organisationen um Standardisierung. Zu erw"ahnen sind
Initiativen vonn IEEE (P1363), ASC (ANSI X9.62, X9.63), IOS/IEC sowie der RSA labs und von Certicom. Im
...
Dies gilt insbesondere, wenn es sich um Anwendung im Finanzsektor handelt, die voraussichtlich noch
