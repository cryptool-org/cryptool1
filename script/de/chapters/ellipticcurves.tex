...
Wichtigkeit. Viele Anwendungen verwenden den RSA-Algorithmus als asymmetrisches Signatur und Verschl"usselungsverfahren.

Solange die verwendeten Schl"ussel hinreichend lang sind, bestehen keinerlei Bedenken gegen die
...
und dem Rechenaufwand ab, um ein vorgeschriebenes Sicherheitsniveau zu erreichen.
Der entscheidende Vorteil Elliptischer Kurven im Vergleich zu RSA-Algorithmus liegt in der Tatsache,
dass die sicheren Schl"ussell"angen erheblich k"urzer sind. 
...
fort, so kann man von einer Entwicklung der sicheren Schl"ussell"angen
wie in Abbildung~\ref{RSAKeylength} ausgehen \cite{Lenstra1999}
...
Da bei Smartcards, die auf RSA basieren, stets der lange (private) Schl"ussel auf der Karte gespeichert werden
...
Aktuelle Informationen zur Sicherheit des RSA-Verfahrens finden Sie in Kapitel \ref{SecurityRSA}}, laufende RSA-Anwendungen umzustellen, sollte man bei der
...
nach 2005\footnote{%
BSI Empfehlung} operativ sein sollen.
...
\begin{itemize}
   ...
   \item F"ur alle Elemente $a, b$ und $c$ aus $G$ gilt: $(ab)c = a(bc)$.
   ...
   \item Zu jedem Element $a$ aus $G$ gibt es ein {\it inverses Element} $a^{-1}$(in $G$), so dass gilt: $aa^{-1} = a^{-1}a = e$.
\end{itemize}
Gilt zus"atzlich noch f"ur alle $a, b$ aus $G$, dass $ab = ba$, so nennt man die Gruppe $G$ eine {\em abelsche} Gruppe.
...
Es gibt auch {\em endliche} Gruppen. Das hei"st es gibt eine Menge $\mathcal{M}$ mit einer endlichen Anzahl von
Elementen und einer Verkn"upfung, so dass obige Bedingungen erf"ullt sind. Ein Beispiel hierf"ur ist
...
Als {\it zyklische Gruppen}\footnote{Zyklische Gruppen sind Gruppen, die von einem Element erzeugt werden. Sie k"onnen grunds"atzlich auch unendlich sein wie z.B. die additive Gruppe der ganzen Zahlen. Wir betrachten hier jedoch nur endliche zyklische Gruppen.} bezeichnet man solche Gruppen $G'$, die ein Element $g$ besitzen, aus dem man
...
In praktischen Anwendungen betrachtet man h"aufig Mengen, auf denen nicht nur eine \mbox{(Gruppen-)} Verkn"upfung, sondern zwei Verkn"upfungen definiert sind. Diese nennt man oft Addition und Multiplikation. Die mathematisch interessantesten Mengen dieser Art sind sogenannte K"orper, wie z.B. die Menge der reellen Zahlen.

Unter einem K"orper versteht man in der Mathematik eine Menge $K$ mit zwei Verkn"upfungen Addition und Multiplikation (mit $+$ und $\cdot$ bezeichnet), so dass die folgenden Bedingungen erf"ullt sind:
\begin{itemize}
   ...
   \item Die Menge $K\setminus\{0\}$ (d.h. $K$ ohne das Element 0) ist zusammen mit der Verkn"upfung $\cdot$ (Multiplikation)
         ebenfalls eine abelsche Gruppe. Dabei sei $1$ das neutrale Element der Verkn"upfung $\cdot$.
   \item F"ur alle Elemente $a, b$ und $c$ aus $K$ gilt $c\cdot (a+b) = c \cdot a + c \cdot b$ und
...
$$ \underbrace{1 + 1 + \cdots + 1}_{\hbox{$n$ mal}} =0
 \, .
$$
Gibt es keine solche nat"urliche Zahl, d.h. ist $1 + 1 + \cdots + 1$ unabh"angig von der Zahl der Summanden nie $0$, so sagt man, der K"orper habe Charakteristik $0$.
...
\index{Endliche!K"orper}
\paragraph{Endliche K"orper}
Wie bereits erw"ahnt, hat jeder endliche K"orper eine Charakteristik $p\ne 0$, wobei $p$ eine Primzahl ist. Zu jeder Primzahl $p$ gibt es genau einen K"orper mit $p$ Elementen\footnote{F"ur Primk"orper sind additive wie multiplikative Gruppe zyklisch.}, n"amlich ${\mathbb Z}_p$. Man nennt ${\mathbb Z}_p$ deshalb auch den Primk"orper zur Primzahl $p$.

Die Anzahl der Elemente eines K"orpers muss jedoch im allgemeinen keine Primzahl sein. So ist es nicht schwer, einen K"orper mit $4$ Elementen zu konstruieren. Man kann allgemein zeigen, dass es genau dann einen K"orper mit $k$ Elementen gibt, wenn $k$ eine Primzahlpotenz ist, sich also in der Form $k=p^n$, $p$ prim, schreiben l"asst. Da zwei endliche K"orper mit gleicher Zahl von Elementen nicht unterscheidbar\footnote{Sind $K,K'$ zwei K"orper mit $k=p^n$ Elementen, so gibt es eine eineindeutige Abbildung $\varphi:K\to K'$, die sich mit der K"orperarithmetik vertr"agt. Eine solche Abbildung nennt man Isomorphie. Isomorphe K"orper verhalten sich mathematisch gleich, so dass es keinen Sinn macht, zwischen ihnen zu unterscheiden. Z.B. sind ${\mathbb Z}_2$ und $K'=\{ NULL,EINS\}$ mit Nullelement $NULL$ und Einselement $EINS$ isomorph.} sind, spricht man von {\bf dem\ K"orper mit $p^n$ Elementen} und bezeichnet diesen mit $GF(p^n)$. Dabei steht $GF$ f"ur {\it Galois Feld} in Erinnerung an den franz"osischen Mathematiker Galois.


% --------------------------------------------------------------------------------------------------------------------
\subsection{Elliptische Kurven in der Kryptographie}

In der Kryptographie betrachten wir Elliptische Kurven. Solche Kurven ergeben sich als L"osungen einer Gleichung. Es hat sich bei Untersuchungen gezeigt, dass die durch eine Kurve vom Grad drei\footnote{d.h. die Summe der Exponenten jedes einzelnen Summanden ist maximal $3$, bei mindestens einem Summanden kommt $3$ auch wirklich vor} der Form
$$ F(x_1,x_2,x_3)=-x_1^3+x_2^2x_3+a_1x_1x_2x_3-a_2x_1^2x_3+a_3x_2x_3^2-a_4x_1x_3^2-a_6x_3^3=0,
$$
wobei die Variablen $x_1,x_2,x_3$ wie die Parameter $a_1,\dots,a_4,a_6$ Elemente eines gegebenen K"orpers $K$ sind, eine f"ur die Kryptographie interessante Eigenschaften besitzt. Der zugrunde liegende K"orper $K$ kann einfach die bekannte Menge der reellen Zahlen oder auch ein endlicher K"orper sein (vgl. letzter Abschnitt).
...
$$
Ferner identifizieren wir Punkte, die sich nur durch eine Vervielfachung jeder Komponente ergeben, denn mit $(x_1,x_2,x_3)$ erf"ullt stets auch $\alpha (x,y,z)$ die Ausgangsgleichung. Formal betrachten wir daher "Aquivalenzklassen von Punkten $(x_1,x_2,x_3)$, wobei wir zwei Punkte als gleich ansehen, wenn sie durch Multiplikation mit einer Konstante $\alpha$ auseinander hervorgehen.
\\ Setzt man in der Ausgangsgleichung $x_3=0$, so wird diese zu $-x_1^3=0$, also $x_1=0$. Folglich ist die "Aquivalenzklasse, die das Element $(0,1,0)$ enth"alt, die einzige Punkt mit $x_3=0$. F"ur allen anderen L"osungspunkte k"onnen wir die Transformation
$$ (x_1,x_2,x_3) \mapsto (x,y):=\left( {x_1\over x_3}, {x_2\over x_3}\right)
$$
vornehmen, die die Anzahl der Variablen von drei auf zwei reduziert. Die Ausgangsgleichung $F(x_1,x_2,x_3)=0$ war so gew"ahlt, dass sich auf diese Weise die sogenannten Weierstrass--Gleichung
\begin{equation}
...
Als zugrunde liegenden K"orper f"ur eine Elliptische Kurve verwendet man in der Kryptographie stets endliche K"orper $K=GF(p^n)$. In der Praxis hat es sich als sinnvoll erwiesen, entweder $GF(p)$ mit einer gro"sen Primzahl $p$ oder $GF(2^n)$ mit einer (gro"sen) nat"urlichen Zahl $n$ zu betrachten. Der Primk"orper $GF(p)$ bietet einerseits eine einfache Arithmetik, andererseits kommt $GF(2^n)$ der bin"aeren Darstellung in Computersystemen entgegen.

Durch Koordinatentransformation kann man die Weierstrass-Gleichung in einer ver"anderten Form schreiben. Mathematische Untersuchungen, auf die wir hier nicht im Detail eingehen k"onnen, haben gezeigt, dass man die Weierstrass-Gleichung in Abh"angigkeit vom zugrunde liegenden K"orper in eine einfachere Form "uberf"uhren kann, ohne relevante Informationen zu verlieren.

Im Fall $GF(p)$, $p>3$, betrachtet man so meist Elliptische Gleichungen der Form
\begin{equation}
 y^2 = x^3 + ax + b
\label{ellp}
\end{equation}
mit $4a^3+27b^2\ne 0$; im Fall $GF(2^n)$ Gleichungen der Form
\begin{equation}
 y^2+xy = x^3 + ax^2 + b
\label{ell2}
\end{equation}
mit $b\ne 0$\footnote{Diese Form ist die Standardform der Weierstrass-Gleichung. Ist die Charakteristik des K"orpers jedoch $2$ oder $3$, so ist $4=0$ bzw. $27=0$, was dazu f"uhrt, dass man in der Bedingung an die Parameter $a,b$ wesentliche Informationen verliert. Dies ist ein Hinweis darauf, dass die Transformation auf die Standardform in diesen F"allen nicht zu befriedigenden Ergebnissen f"uhrt.}. Durch diese Bedingungen an die Parameter $a,b$ ist gew"ahrleistet, dass die Elliptische Gleichung f"ur die kryptographische Anwendungen geeignet ist\footnote{formal sagt man, die Kurve ist nicht singul"ar}.

F"ur die Anzahl $|E|$ der Elemente einer Elliptischen Kurve $E$ "uber einem K"orper $GF(k)$ (praktisch $k=p$ prim oder $k=2^n$) gilt die einfache Beziehung
...
Um mit Elliptischen Kurven arbeiten zu k"onnen, definiert man eine Verkn"upfung (meist additiv als $+$ geschrieben) auf den Punkten der Elliptischen Kurve. Dabei definiert man bei Elliptischen Kurven "uber $GF(p)$ die Verkn"upfung durch
\begin{enumerate}
...
Besonders interessant sind aus kryptographischer Sicht Vielfache eines gegebenen Punktes $P\in E$. So bezeichnet man f"ur eine nat"urliche Zahl $m$
$$ mP := \underbrace{P+P+\dots+P}_{\hbox{$m$ mal}} .
$$
Um $mP$ zu berechnen, muss man im allgemeinen $\log m$ Additionen durchf"uhren --- man bildet einfach $P$, $2P$, $2^2P$, $2^3P$, \dots, schreibt $m$ bin"ar und addiert schlie"slich entsprechend der Bin"ardarstellung von $m$ auf.
Andererseits erscheint es extrem schwierig zu sein, zu gegebenen Punkten $P$ und $Q=mP$ auf $E$ die Zahl $m$ zu bestimmen. Nat"urlich kann man $P,2P,3P,4P,5P,\dots$ bilden und jeweils mit $Q$ vergleichen. Hierzu ben"otigt man jedoch $m$ Additionen.
...
\end{sloppypar}
\begin{itemize}
    \item Eine Elliptischen Kurve {\bf E}, beschrieben durch den zugrunde liegenden K"orper $GF(k)$, "uber dem die Elliptische Kurve ${\bf E}$ definiert ist, und die Kurvenparameter.
...
Mit Hilfe Elliptischer Kurven kann ein Schl"usselaustausch nach dem Dieffie-Helman Protokoll erfolgen. Zun"achst einigen sich beide Partner (A und B) "offentlich auf eine Gruppe $G$ und ein Element $q$. Danach w"ahlen sie zuf"allig $r_A,r_B\in\{1,2,\dots,q-1\}$, bilden $R_A=r_AG$, $R_B=r_BG$ und tauschen diese aus. Danach berechnet A leicht $R=r_AR_B$. Denselben Punkt $r_Ar_B G$ erh"alt auch B, indem er $r_BR_A$ bildet.
...
"Ubertr"agt man den DSA auf Elliptische Kurve, so kann man wie folgt eine digitale Signatur erzeugen: Man w"ahlt vorab eine (nicht-triviale) Zahl $s\in{\mathbb Z}_q$. Diese bildet den privaten Schl"ussel. Hingegen werden $q$, $G$ und $R=sG$ ver"offentlicht. Aus $G$ und $R$ l"asst sich jedoch $s$ nicht ermitteln, worauf die Sicherheit des Signaturverfahrens beruht.
...
$$ rx-s\bar r_1-h(m)=0 .
$$
Das Trippel $(m,r_1,x)$ bildet nun die digitale Signatur.
...
Es gibt Faktorisierungsalgorithmen, die auf Elliptischen Kurven
\index{Faktorisierung!Faktorisierungsalgorithmen}
...
(siehe \cite{Lenstra1987}). Die aktuell gr"o"ste mit Elliptischen Kurven faktorisierte Zahl\index{Faktorisierung!Faktorisierungsrekorde} ist die 55 Dezimalstellen lange Zahl $629^{59}-1$, sie wurde am 6. Oktober 2001 von M. Izumi gefunden (siehe \hyperlink{Lenstra2}{ECMNET}\index{ECMNET}).}.
...
Eine vollst"andige Beschreibung aller Implementierungen unter Abw"agen ihrer
Vor- und Nachteile w"urde an dieser Stelle zu weit f"uhren. Abschlie"send kann
festgehalten werden, dass die Vielfalt m"oglicher Implementierungen bei
Elliptischen Kurven z.B. im Vergleich zu RSA-Implementie"-rungen sehr gro"s ist.
Aus diesem Grund gibt es Bestrebungen, sich auf wenige
Standardimplementierungen, ja sogar auf eine kleine Schar fest vorgegebener
Kurven zu beschr"anken (ASC-Ansatz).

Der Erfolg dieser Standardisierungsbem"uhungen ist heute noch nicht absehbar. Dies w"are aber eine Voraussetzung daf"ur, dass sich ECC dauerhaft als Alternative zu RSA etablieren wird.
...
\addcontentsline{toc}{subsection}{Literaturverzeichnis}
	...
	\bibitem[Lenstra1999]{Lenstra1999} Arjen K. Lenstra, Eric R. Verheul \\ \index{Lenstra/Verheul 1999}
		...
		\href{cryptosavvy.com/cryptosizes.pdf}{\texttt{cryptosavvy.com/cryptosizes.pdf}}
\end{thebibliography}


\section*{B"ucher "uber Elliptische Kurven (Auswahl)}\addcontentsline{toc}{subsection}{B"ucher "uber Elliptische Kurven (Auswahl)}
\begin{itemize}
    \item[[{\sc Cas}]] J. W. S. Cassels, {\em Lectures on elliptic curves}, Cambridge University Press, 1991, 143 Seiten.
    \item[[{\sc Kob}]] N. Koblitz, {\em Introduction to elliptic curves and modular forms}, Graduate Texts in Mathemathics, Springer-Verlag, 1984.
    \item[[{\sc Kob}]] N. Koblitz, {\em Algebraic aspects of Cryptography. With an appendix on Hyperelleptic curves by Alfred J. Menezes, Yi Hong Wu and Robert J. Zuccherato}, Springer, 1998, 206 Seiten.
    \item[[{\sc Men}]] A. J, Menezes, {\em Elliptic curve public key cryptosystems}, Kluwer Academic Publishers, 1993.
    \item[[{\sc Sil}]] J. Silverman, {\em The arithmetic of elliptc curves}, Graduate Texts in Mathemathics, Springer-Verlag, 1992.
    \item[[{\sc ST}]] J. Silverman, J. Tate, {\em Rational points on elliptic curves}, Springer-Verlag, 1992.
\end{itemize}
