...
Der Begriff \glqq praktisch unm"oglich\grqq {} ist etwas schwammiger. Allgemein
kann man sagen, ein Problem ist \index{Laufzeit!effizient} nicht effizient
l"osbar, wenn der zu seiner L"osung ben"otigte Aufwand schneller w"achst als die
polynomiale Zeit als Funktion der Gr"o"se der Eingabe. Wenn beispielsweise die
L"ange der Eingabe $ n $  Bits betr"agt und die Zeit  zur Berechnung der
Funktion proportional zu $ 2^n $ ist, so gilt zur Zeit: die Funktion ist f"ur $n
> 80$ praktisch nicht zu berechnen (f"ur $ a=5 $ gilt: ab der L"ange $ n=23 $
ist $ 2^n > n^5 $~\mbox{und} danach w"achst $ 2^n $ auch deutlich schneller ($
2^{23}=8.388.608, $ $ 23^5= 6.436.343 $)).\par 
