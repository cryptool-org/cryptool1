...
\addtocounter{footnote}{0}\footnotetext{Daniel Suarez, \glqq Darknet\grqq, rororo,
                           (c) 2011, Kapitel 5, \glqq Einsichten\grqq, S. 69, Price.}
...
den CT-Programmen, im Kapitel~\ref{Kapitel_PaperandPencil}
(\glqq \nameref{Kapitel_PaperandPencil}\grqq ) in diesem Skript oder 
in \cite{cm:Nichols1996}. 
In diesem Absatz wollen wir jedoch nur die moderneren symmetrischen Verfahren
betrachten.
...
Für größere Formeln ist es nicht so einfach herauszufinden, ob eine Formel zu 1 ausgewertet werden kann (dieses Problem gehört zu den NP-vollständigen Problemen). Daher wurden spezielle Tools entwickelt, um dieses Problem für generelle Boolesche Formeln zu lösen, sogenannte SAT-Solver\footnote{
    ...
    SAT-Solver (Texteingabe)}  und  {\bf SAT-Solver (Dateieingabe)} einen SAT-Solver
    aufrufen.
...
Unter kontrollierten Bedingungen wurden Preise ausgelobt, um Ciphertexte
...
öffentlicher Schlüsselring genannt).\par \vskip + 3pt
...
erläutert. Neben den
Anforderungen bei der Schlüsselgenerierung ist auch zu beachten, dass
...
\bibitem[Schmeh2012]{cm:Schmeh2012}  \index{Schmeh 2012}
       Klaus Schmeh, \\
       {\em Kryptografie -- Verfahren, Protokolle, Infrastrukturen},\\ 
       dpunkt.verlag, 5. aktualisierte und erweiterte Auflage 2012. \\
