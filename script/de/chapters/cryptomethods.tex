...
Juni 2005, Juli 2007, Jan. 2010, Nov. 2012)
...
Zur Erläuterung verwenden wir die Begriffe aus der folgenden Abbildung \ref{Generic-Notations-when-Encrypting}:
\begin{figure}[ht]
\begin{center}
...
gebrochen werden können, auch nicht mit unendlich viel Rechenkraft -- ein Beispiel
dafür ist das {\em One-Time-Pad}\index{One-Time-Pad} (OTP\index{OTP}).
...
Beispiel: Aktuell wissen wir, dass RSA höchstens so schwierig ist wie die
Faktorisierung, aber wir können nicht beweisen, dass es genauso schwierig ist.
...
Beispiel: Dies kann z.B. zutreffen, wenn Börsen-relevante Daten sowieso am
nächsten Tag veröffentlicht werden und man für das Brechen ein Jahr brauchen
würde.
...
Algorithmus bezogenen Sichtweise.%\par \vskip + 3pt


\begin{sloppypar}
Einen guten Überblick über die verschiedenen Verfahren bietet das Buch von Bruce
...
{Symmetrische Verschlüsselung\footnotemark}
  \footnotetext{% XXX dieses Prozent ist nötig, sonst startet die Fußnote mit einem Blank !
    Mit CrypTool 1 ({\bf CT1})\index{CrypTool 1} können Sie über das Menü
    ...
    Mit CrypTool 2 ({\bf CT2})\index{CrypTool 2} können Sie im Startcenter
    ...
    In JCrypTool ({\bf JCT})\index{JCrypTool} stehen Ihnen die folgenden modernen
    symmetrischen Verschlüsselungsverfahren zur Verfügung:\\ 
...
der letzten Runde finden Sie z.B. in der
CrypTool-1-Online-Hilfe\index{CrypTool 1}%
\footnote{%
...
\footnote{%
  Mit CrypTool 1\index{CrypTool 1} können Sie über das Menü
  ...
  Mit CrypTool 2\index{CrypTool 2} können Sie bei den Vorlagen unter
...
sogenannte Cipher-Challenges\footnote{%
 ...
 Im Mai 2007 meldete RSA, dass sie die Korrektheit der bis dahin nicht
 gelösten RC5-72 Challenge nicht bestätigen werden.}.
...
{Asymmetrische Verschlüsselung\footnotemark}
  \footnotetext{%
   ...
   JCrypTool\index{JCrypTool} bietet asymmetrische Verfahren wie RSA sowohl im
   Menü {\bf Visualisierungen} der Standard-Perspektive als auch in der Funktionalen
   Perspektive.
  }
...
\footnotetext{%
   ...
   JCrypTool\index{JCrypTool} bietet Hybrid-Verfahren (z.B. ECIES) in der
   Funktionalen Perspektive an unter {\bf Algorithms \textbackslash{} Hybrid Ciphers}.
}
...
verschlüsselt: Der Schlüssel ist ein vom Absender zufällig\footnote{%
   ...
   anwenden. \\
   CT1 konzentriert sich auf kryptographisch starke {\bf Pseudo}zufallszahlengeneratoren.
   Nur im Secude-Generator wird eine "`echte"' Zufallsquelle einbezogen.\\
   JCrypTool\index{JCrypTool} bietet {\bf Pseudo}zufallszahlengeneratoren sowohl
   im Menü {\bf Algorithms \textbackslash{} Random Number Generator} der
   Standard-Perspektive als auch in der Funktionalen Perspektive an.
}\index{Zufall}
