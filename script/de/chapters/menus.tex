...
CrypTool\index{CrypTool}-Version 1.4.31\footnote{%
  ...
  CrypTool~1 (\textbf{CT1})\index{CT1} vor allem auf Bugfixes
  beschränkten, fließen viele Neuerungen in die beiden Nachfolgeversionen
  CrypTool~2 (\textbf{CT2})\index{CT2} und JCrypTool (\textbf{JCT})\index{JCT} ein:\\
...
Das Haupt-Menü von CT1 enthält die generellen Service-Funktionen
...
Dieser Anhang enthält auf den folgenden Seiten den Baum
...
Dieser Anhang enthält auf den folgenden Seiten eine Liste aller
...
Alle Funktionen in der \textbf{Standard-Perspektive} finden sich sowohl in den Menüs als
...
Alle Funktionen der \textbf{Algorithmen-Perspektive} finden sich in der \glqq Algorithmen\grqq~genannten Navigationsleiste (in dieser Perspektive ebenfalls rechts). Die Algorithmen-Perspektive enthält alle Detaileinstellungen der verschiedenen Algorithmen und bietet insbesondere auch Algorithmen aus dem Bereich des Post-Quantum-Computings\index{Post-Quantum-Computing} (PQC) an.
...
Dieser Anhang enthält eine Liste aller
...
%Die Einstiegsseite von CTO sieht so aus:
%\clearpage
%\begin{figure}[hb]
%\begin{center}
%\vspace{-30pt}
...
Der folgende Screenshot zeigt die auf CTO implementierten
Krypto-Funktionen:
\clearpage
\begin{figure}[hb]
\begin{center}
