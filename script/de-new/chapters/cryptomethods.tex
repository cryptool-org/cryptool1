...
% $Id: cryptomethods.tex 3796 2017-03-23 11:06:02Z esslinger $
...
Sinn der Verschlüsselung \index{Verschlüsselung} ist es, Daten so zu verändern, dass nur ein autorisierter Empfän\-ger in der Lage ist, den Klartext zu rekonstruieren. 
...
\item {\bfseries  Beweisbare Sicherheit}
...
\item {\bfseries  Ad-hoc-Sicherheit}
...
besser beherrschte konnte mit Hilfen der Geheimdienste großen Einfluss auf die {\bfseries Politik}
...
\section[Symmetrische Verschlüsselung] % Was zwischen [...] steht, kommt ins Inhaltsverzeichnis!
  ...
  \footnotetext{% Dieses Prozent ist nötig, sonst startet die Fußnote mit einem Blank !
    Mit CrypTool~1 ({\bfseries CT1})\index{CT1} können Sie über das
    Menü {\bfseries Ver-/Entschlüsseln \textbackslash{} Symmetrisch (modern)} folgende
    ...
    Mit CrypTool~2 ({\bfseries CT2})\index{CT2} können Sie im Startcenter
    über {\bfseries Vorlagen \textbackslash{} Kryptographie \textbackslash{}
    ...
    In JCrypTool ({\bfseries JCT})\index{JCT} stehen Ihnen die folgenden
...
\footnotetext{%
   ...
   über das Menü {\bfseries Einzelverfahren \textbackslash{} Visualisierung von
...
\footnote{%
      Online-Hilfe von CT1\index{CT1}: Das Stichwort {\bfseries AES} im Index
...
eines Blocks in {\bfseries CT2}\index{CT2}.
...
\vspace{15pt}
\noindent Dasselbe Ergebnis kann man mit {\bfseries OpenSSL}\index{OpenSSL}%
\footnote{%
...
Für größere Formeln ist es nicht so einfach herauszufinden, ob eine Formel zu 1 ausgewertet werden kann (dieses Problem gehört zu den NP-vollständigen Problemen). Daher wurden spezielle Tools entwickelt, um dieses Problem für generelle Boolesche Formeln zu lösen, sogenannte SAT-Solver\footnote{
    Mit {\bfseries CT2}\index{CT2} können Sie im Startcenter
    über {\bfseries Vorlagen \textbackslash{} Mathematisch \textbackslash{}
    SAT-Solver (Texteingabe)}  und  {\bfseries SAT-Solver (Dateieingabe)} einen
    SAT-Solver aufrufen.
...
\footnote{%
  ...
  {\bfseries Analyse \textbackslash{} Symmetrische Verschlüsselung (modern)}
  ...
  {\bfseries Kryptoanalyse \textbackslash{} Modern} ebenfalls Brute-Force-Analysen
...
sogenannte Cipher-Challenges\index{Challenge}.\footnote{%
  ...
  {\bfseries MysteryTwister C3}: %% \glqq MysteryTwister C3\grqq:
...
{Asymmetrische Verschlüsselung\footnotemark}
  \footnotetext{%
   ...
   {\bfseries Einzelverfahren \textbackslash{} RSA-Kryptosystem \textbackslash{} 
   RSA-Demo} in vielen Variationen nachvollzogen werden.\\
   Mit CT1\index{CT1} können Sie über das Menü
   {\bfseries Ver-/Entschlüsseln \textbackslash{} Asymmetrisch} mit RSA 
   ...
   Mit CT2\index{CT2} können Sie bei den Vorlagen unter
   {\bfseries Kryptographie \textbackslash{} Modern} ebenfalls asymmetrische
   Verfahren durchführen.\\
   JCT\index{JCT} bietet asymmetrische Verfahren wie RSA sowohl im
   Menü {\bfseries Visualisierungen} der Standard-Perspektive als auch in der
...
\footnotetext{%
   ...
   {\bfseries Ver-/Entschlüsseln \textbackslash{} Hybrid}:
   ...
   Algorithmen-Perspektive an unter {\bfseries Algorithms \textbackslash{} Hybrid Ciphers}.
}
...
verschlüsselt: Der Schlüssel ist ein vom Absender zufällig\footnote{%
   ...
   %
   - JCT\index{JCT} bietet {\bfseries Pseudo}zufallszahlengeneratoren sowohl
   im Menü {\bfseries Algorithms \textbackslash{} Random Number Generator} der
