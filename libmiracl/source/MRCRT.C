...
{ /* Chinese Remainder Thereom                  *
   * Calculate x given remainders u[i] mod M[i] */
    ...
    { /* Knuth page 274 */
        ...
        if (size(c->V[i])<0) add_r(_MIPP_ c->V[i],c->M[i],c->V[i]);
    }
    zero(x);
    ...
    for (i=0;i<c->NP;i++)
    {
        ...
        add_r(_MIPP_ x,mr_mip->w2,x);         
        multiply(_MIPP_ mr_mip->w1,c->M[i],mr_mip->w1);
    }   
    MR_OUT
}

void crt_end(_MIPD_ big_chinese *c)
{ /* clean up after CRT */
    int i,j,k;
    if (c->NP<2) return;
    for (k=0,i=0;i<c->NP;i++)
    {
        mirkill(_MIPP_ c->M[i]);
        for (j=0;j<i;j++)
            mirkill(_MIPP_ c->C[k++]); 
        mirkill(_MIPP_ c->V[i]);
    }   
    mr_free(_MIPP_ c->M);
    mr_free(_MIPP_ c->V);
    mr_free(_MIPP_ c->C);
