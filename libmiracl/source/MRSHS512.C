...
static void shs_transform(sha_r512 *sh)
...
void shs512_process(sha_r512 *sh,int byte)
...
void shs384_process(sha_r384 *sh,int byte)
{ /* process the next message byte */
    int cnt;
...
void shs512_hash(sha_r512 *sh,char hash[64])
{ /* pad message and finish - supply digest */
    ...
    for (i=0;i<64;i++)
    { /* convert to bytes */
        hash[i]=((sh->h[i/8]>>(8*(7-i%8))) & 0xffL);
    }
    shs512_init(sh);
}

void shs384_hash(sha_r384 *sh,char hash[48])
{ /* pad message and finish - supply digest */
    int i;
    mr_unsign64 len0,len1;
    len0=sh->length[0];
