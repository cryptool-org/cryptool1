...
inline int CPictureExLoop::TGIFControlExt::GetPackedValue(enum ControlExtValues Value)
{
...
inline int CPictureExLoop::TGIFLSDescriptor::GetPackedValue(enum LSDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
inline int CPictureExLoop::TGIFImageDescriptor::GetPackedValue(enum IDPackedValues Value)
{
	int nRet = (int)m_cPacked;
...
BEGIN_MESSAGE_MAP(CPictureExLoop, CStatic)
	//{{AFX_MSG_MAP(CPictureExLoop)
	ON_WM_DESTROY()
	ON_WM_PAINT()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()

BOOL CPictureExLoop::Load(HGLOBAL hGlobal, DWORD dwSize)
{
...
BOOL CPictureExLoop::Draw()
{
	if (!m_bIsInitialized)
	{
		TRACE(_T("Call one of the CPictureExLoop::Load() member functions before calling Draw()\n"));
		return FALSE;
	};
...
SIZE CPictureExLoop::GetSize() const
{
	return m_PictureSize;
}

BOOL CPictureExLoop::Load(LPCTSTR szFileName)
{
	ASSERT(szFileName);
...
BOOL CPictureExLoop::Load(LPCTSTR szResourceName, LPCTSTR szResourceType)
{
...
void CPictureExLoop::ResetDataPointer()
{
...
BOOL CPictureExLoop::SkipNextGraphicBlock()
{
...
UINT CPictureExLoop::GetSubBlocksLen(UINT nStartingOffset) const
{
	UINT nRet = 0;
...
enum CPictureExLoop::GIFBlockTypes CPictureExLoop::GetNextBlock() const
{
...
BOOL CPictureExLoop::SkipNextBlock()
{
	if (!m_pRawData) return FALSE;
...
int CPictureExLoop::GetNextBlockLen() const
{
...
UINT WINAPI CPictureExLoop::_ThreadAnimation(LPVOID pParam)
{
	ASSERT(pParam);
	CPictureExLoop *pPic = reinterpret_cast<CPictureExLoop *> (pParam);
...
void CPictureExLoop::ThreadAnimation()
{
...
HGLOBAL CPictureExLoop::GetNextGraphicBlock(UINT *pBlockLen, 
...
BOOL CPictureExLoop::IsGIF() const
{
	return m_bIsGIF;
}

BOOL CPictureExLoop::IsAnimatedGIF() const
{
	return (m_bIsGIF && (m_arrFrames.size() > 1));
}

BOOL CPictureExLoop::IsPlaying() const
{
	return m_bIsPlaying;
}

int CPictureExLoop::GetFrameCount() const
{
	if (!IsAnimatedGIF())
		return 0;
...
COLORREF CPictureExLoop::GetBkColor() const
{
	return m_clrBackground;
}

void CPictureExLoop::OnPaint() 
{
...
BOOL CPictureExLoop::PrepareDC(int nWidth, int nHeight)
{
...
void CPictureExLoop::OnDestroy() 
{
	Stop();	
	CStatic::OnDestroy();
}

void CPictureExLoop::SetBkColor(COLORREF clr)
{
...
#ifdef GIF_TRACING
void CPictureExLoop::WriteDataOnDisk(CString szFileName, HGLOBAL hData, DWORD dwSize)
{
	CFile file;
...
void CPictureExLoop::EnumGIFBlocks()
{
	enum GIFBlockTypes nBlock;
...
BOOL CPictureExLoop::SetPaintRect(const RECT *lpRect)
{
	return CopyRect(&m_PaintRect, lpRect);
}

BOOL CPictureExLoop::GetPaintRect(RECT *lpRect)
{
